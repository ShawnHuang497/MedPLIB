...
from ..medplib_arch import LlavaMetaModel, LlavaMetaForCausalLM
...
class MedPLIBMoELlamaConfig(LlamaConfig):
    ...
    def __init__(self,
        ...
        super(MedPLIBMoELlamaConfig, self).__init__(**kwargs)


class MedPLIBMoELlamaModel(LlavaMetaModel, LlamaModel):
    config_class = MedPLIBMoELlamaConfig
...
class MedPLIBMoELlamaForCausalLM(LlamaForCausalLM, LlavaMetaForCausalLM):
    config_class = MedPLIBMoELlamaConfig

    def __init__(self, config):
        super(LlamaForCausalLM, self).__init__(config)
        self.model = MedPLIBMoELlamaModel(config)
    ...
    #     # print(_inputs['images'], '_inputs images')
    #     return _inputs
    ...
    def initialize_moe_modules(self, model_args):
        expert_pretrained_path_lst = model_args.expert_pretrained_path.split(',')
        print('expert_pretrained_path_lst', expert_pretrained_path_lst)


        expert_state_dict = {}
        for idx, expert_pretrained_path in enumerate(expert_pretrained_path_lst):
            assert os.path.exists(expert_pretrained_path), f'{expert_pretrained_path} does not exist'
            tmp_dict = {}
            bin_files = glob.glob(os.path.join(expert_pretrained_path, '*.bin'))
            for file_path in bin_files:  
                data = torch.load(file_path) 
                tmp_dict.update(data) 
                
            if getattr(model_args, 'finetune_moe', False):
                # finetune on downstream datasets
                for k, v in tmp_dict.items():
                    if any([target in k for target in ['gate_proj','up_proj','down_proj']]):
                            expert_state_dict[k] = v

                target_dict = {}
                for k, v in tmp_dict.items():
                    if any([target in k for target in ['text_hidden_fcs']]):
                        target_dict[k.replace('model.text_hidden_fcs.', '')] = v
                load_res = self.model.text_hidden_fcs.load_state_dict(target_dict, strict=False)
                print('the text projector weights load results', load_res)

                target_dict = {}
                for k, v in tmp_dict.items():
                    if any([target in k for target in ['mask_decoder']]):
                        target_dict[k.replace('model.visual_model.mask_decoder.', '')] = v
                load_res = self.model.visual_model.mask_decoder.load_state_dict(target_dict, strict=False)
                print('the mask decoder weights load results', load_res)

                target_dict = {}
                for k, v in tmp_dict.items():
                    if any([target in k for target in ['region_fea_adapter']]):
                        target_dict[k.replace('model.region_fea_adapter.', '')] = v
                load_res = self.model.region_fea_adapter.load_state_dict(target_dict, strict=False)
                print('the region feat projector weights load results', load_res)

            else:
                # for stage 4
                expert_state_dict[idx] = {}
                for k, v in tmp_dict.items():
                    if any([target in k for target in ['gate_proj','up_proj','down_proj']]):
                        expert_state_dict[idx][k] = v

            # ------2 experts: load the mask decoder weights, region_fea_adapter------
            if idx == 0:
                # we assume the first model is seg model from stage 3
                target_dict = {}
                for k, v in tmp_dict.items():
                    if any([target in k for target in ['text_hidden_fcs']]):
                        target_dict[k.replace('model.text_hidden_fcs.', '')] = v
            ...
            # load the region feat projector weights
            else:
                # we assume the second model is vl model from stage 2
                target_dict = {}
                for k, v in tmp_dict.items():
                    if any([target in k for target in ['region_fea_adapter']]):
                        target_dict[k.replace('model.region_fea_adapter.', '')] = v
        ...
        for num_experts, layer_num in zip(self.config.moe['num_experts'], moe_layers_idx):
            ...
            # load different expert state_dict
            for e_idx, e in enumerate(self.model.layers[layer_num].mlp.deepspeed_moe.experts.deepspeed_experts):
                # print('e.state_dict().keys()',e.state_dict().keys())
                # print('expert_state_dict[e_idx].keys()', expert_state_dict[e_idx].keys())
                if any('lora' in key for key in e.state_dict().keys()):
                    if getattr(model_args, 'finetune_moe', False):
                        # finetune on downstream datasets
                        target_dict = {'down_proj.base_layer.weight': expert_state_dict[f'model.layers.{layer_num}.mlp.deepspeed_moe.experts.deepspeed_experts.{e_idx}.down_proj.weight'], 
                                        'up_proj.base_layer.weight': expert_state_dict[f'model.layers.{layer_num}.mlp.deepspeed_moe.experts.deepspeed_experts.{e_idx}.up_proj.weight'], 
                                        'gate_proj.base_layer.weight': expert_state_dict[f'model.layers.{layer_num}.mlp.deepspeed_moe.experts.deepspeed_experts.{e_idx}.gate_proj.weight']}
                    else:
                        # for stage 4 loading with lora
                        target_dict = {'down_proj.base_layer.weight': expert_state_dict[e_idx][f'model.layers.{layer_num}.mlp.down_proj.weight'], 
                                        'up_proj.base_layer.weight': expert_state_dict[e_idx][f'model.layers.{layer_num}.mlp.up_proj.weight'], 
                                        'gate_proj.base_layer.weight': expert_state_dict[e_idx][f'model.layers.{layer_num}.mlp.gate_proj.weight']}
                else:
                    # for stage 4 loading without lora
                    target_dict = {'down_proj.weight': expert_state_dict[e_idx][f'model.layers.{layer_num}.mlp.down_proj.weight'], 
                                    'up_proj.weight': expert_state_dict[e_idx][f'model.layers.{layer_num}.mlp.up_proj.weight'], 
                                    'gate_proj.weight': expert_state_dict[e_idx][f'model.layers.{layer_num}.mlp.gate_proj.weight']}

                load_res = e.load_state_dict(target_dict, strict=False)
                print(layer_num, e_idx, 'expert load results', load_res) 
...
class EvalMedPLIBMoELlamaForCausalLM(MedPLIBMoELlamaForCausalLM):
    config_class = MedPLIBMoELlamaConfig

    def __init__(self, config):
        super(EvalMedPLIBMoELlamaForCausalLM, self).__init__(config)
...
AutoConfig.register("medplib_moe_llama", MedPLIBMoELlamaConfig)
AutoModelForCausalLM.register(MedPLIBMoELlamaConfig, MedPLIBMoELlamaForCausalLM)

AutoModelForCausalLM.register(MedPLIBMoELlamaConfig, EvalMedPLIBMoELlamaForCausalLM)
