...
def build_sam_vit_b(sam_checkpoint, image_size=256, encoder_adapter=True):
    return _build_sam(
        encoder_embed_dim=768,
        encoder_depth=12,
        encoder_num_heads=12,
...
def _build_sam(
    ...
    if checkpoint is not None:
        ...
        if 'model' in state_dict.keys():
            load_res = sam.load_state_dict(state_dict['model'], False)

            # --------------------load sam strictly----------
            # pretrained_dict = state_dict['model']
            # for name, param in sam.named_parameters():
            #     if 'meta' in param.type():
            #         setattr(sam, name, torch.nn.Parameter(torch.empty_like(param, dtype=param.dtype, device=param.device)))
